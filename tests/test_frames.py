...
def test_set_boxes_replaces_the_whole_list() -> None:
    buffer = FrameBuffer()
    buffer.publish(a_frame())
    buffer.add_box({"name": "Old", "x": 9, "y": 9, "w": 9, "h": 9, "score": 0.5, "tapped": False})

    buffer.set_boxes([
        {"name": "Damage", "x": 1, "y": 2, "w": 3, "h": 4, "score": 0.9, "tapped": False},
        {"name": "Health", "x": 5, "y": 6, "w": 7, "h": 8, "score": 0.8, "tapped": True},
    ])

    names = {box["name"] for box in buffer.boxes()}
    assert names == {"Damage", "Health"}


def test_set_boxes_stores_detached_copies() -> None:
    buffer = FrameBuffer()
    buffer.publish(a_frame())
    source = [{"name": "Damage", "x": 1, "y": 2, "w": 3, "h": 4, "score": 0.9, "tapped": False}]

    buffer.set_boxes(source)
    source.clear()
